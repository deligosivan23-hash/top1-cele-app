...
// Issue #8 fix: import shared constant instead of hardcoding 90.
...
function buildSeedData() {
  ...
  const day = 86400000;
  const seedAttempts = [
    // Issue #8 fix: replace hardcoded 90 with PER_QUESTION_SECONDS.
    ...Array.from({ length: 12 }, (_, i) => ({ id: `seed-alg-${i}`, topicId: 'alg-linear', correct: i < 5, timeSeconds: PER_QUESTION_SECONDS + Math.floor(Math.random() * 60), timestamp: now - day * 2 - i * 3600000 })),
...
// Issue #5 fix: module-level cache so buildSeedData runs at most once per JS
// runtime. Zustand persist will override the seed values with localStorage data
// for returning users — this just prevents redundant Date.now()/Math.random()
// calls on every page load after the first install.
let _seedCache = null;
function getSeedData() {
  if (!_seedCache) _seedCache = buildSeedData();
  return _seedCache;
}
...
export const useStore = create(
  persist(
    (set, get) => {
      // Issue #5 fix: call getSeedData() (cached singleton) instead of
      // buildSeedData() directly, so the expensive random/timestamp computation
      // runs at most once per JS runtime, not on every store initialization.
      const { seedAttempts, seedStats } = getSeedData();
      ...
      // Issue #7 fix: incremental update for the affected topicId only, instead
      // of recomputing all topic stats via recomputeTopicStats on every answer.
      // recomputeTopicStats is kept intact for use by generateDailyPlan and any
      // full-recompute scenarios.
      logAttempt(topicId, correct, timeSeconds) {
        ...
        set((state) => {
          ...
          // Incremental update: only recompute stats for the affected topic.
          ...
          const updatedStat   = {
            ...
            accuracy:      Math.round((correctCount / topicAttempts.length) * 100),
            avgTime:       Math.round(topicAttempts.reduce((s, a) => s + a.timeSeconds, 0) / topicAttempts.length),
            lastPracticed: Math.max(...topicAttempts.map((a) => a.timestamp)),
          };
          const topicStats  = { ...state.topicStats, [topicId]: updatedStat };
          const studyTasks  = state.studyTasks.map((task) =>
            task.topicId === topicId ? { ...task, doneCount: task.doneCount + 1 } : task
          );
    ...
    }, // close factory function
    {
      ...
      // Issue #5 fix: onRehydrateStorage confirms the intent — when _seeded is
      // true in the rehydrated state, persist has overridden the getSeedData()
      // initial values and no further action is needed. When _seeded is false
      // (first install or after resetAll), the seed initial state remains.
      onRehydrateStorage: () => (_state) => {
        // No action required: getSeedData() singleton + persist middleware
        // already handle the correct state for both new and returning users.
      },
    }
  )
);
