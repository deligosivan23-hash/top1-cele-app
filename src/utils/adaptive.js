...
import { PER_QUESTION_SECONDS } from './constants';

// ─── Internal scoring helpers ─────────────────────────────────────────────────

/**
 * Split attempts for a topic into timed (<= PER_QUESTION_SECONDS) and
 * untimed (> PER_QUESTION_SECONDS) buckets.
 */
function splitTimedUntimed(topicAttempts) {
  const timed   = topicAttempts.filter((a) => a.timeSeconds <= PER_QUESTION_SECONDS);
  const untimed = topicAttempts.filter((a) => a.timeSeconds >  PER_QUESTION_SECONDS);
 ...
 */
/**
 * Returns a Set of all L3 topic IDs covered by the given enrolled subject IDs.
 * Uses universitySubjects.coveredTopicIds to resolve subject → topic mappings.
 * Only returns IDs that actually exist as L3 topics in the topics tree.
 *
 * @param {string[]} subjectIds
 * @returns {Set<string>}
 */
function getTopicIdsForSubjects(subjectIds) {
  const l3Set = new Set(getLevel3Topics().map((t) => t.id));
  const allowed = new Set();
  for (const subjectId of subjectIds) {
    const subject = universitySubjects.find((s) => s.id === subjectId);
    if (!subject) continue;
    for (const tid of subject.coveredTopicIds) {
      if (l3Set.has(tid)) allowed.add(tid);
    }
  }
  return allowed;
}
...
export function getAdaptivePracticeQueue(topicStats, attempts, enrolledSubjectIds, count = 5) {
  ...
  // Bug #3 fix: properly filter L3 topics to only those belonging to enrolled
  // subjects. If no subjects are enrolled, all L3 topics remain eligible.
  let eligible = l3;
  if (enrolledSubjectIds && enrolledSubjectIds.length > 0) {
    const allowedIds = getTopicIdsForSubjects(enrolledSubjectIds);
    if (allowedIds.size > 0) {
      eligible = l3.filter((t) => allowedIds.has(t.id));
    }
  }
 ...
 *   - At least 5 timed attempts (<= PER_QUESTION_SECONDS)
