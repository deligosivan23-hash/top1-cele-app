...
export default function PracticeSessionScreen() {
  ...
  function doSubmit(choiceIdx, timedOut = false) {
    ...
    // Haptics + flash
    if (correct) { vibrate(50);           setFlashClass('flash-correct'); }
    else         { vibrate([100,50,100]); setFlashClass('flash-wrong');   }
    ...
    const newResults = (prev => {
      ...
      // auto-advance on correct after 1.5 s
      if (correct) {
        autoAdvRef.current = setTimeout(() => advance(updated), 1500);
      }
      return updated;
    })(results);
  ...
  function advance(latestResults) {
    ...
    const finalResults = latestResults ?? results;
  ...
  function doFinish(finalResults) {
    ...
    navigate('/practice/summary', {
      state: { results: finalResults ?? results, topicId, mode, totalQuestions },
    });
  }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
          <div className="bg-white w-full max-w-[420px] rounded-t-3xl px-5 pt-5 pb-10 shadow-2xl">
            ...
            <p className="text-sm text-gray-400 mb-6">
              {qIndex} of {totalQuestions} answered — progress will be saved.
            </p>
