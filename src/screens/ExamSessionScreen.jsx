...
// Major Bug #5 (hardcoded 90s logAttempt time), Minor Issue #7
// (remove local formatTime — now imported from ../utils/stats), and
// Issue #4 (finishExamRef keeps timer useEffect's finishExam call always current).
...
export default function ExamSessionScreen() {
  ...
  // Issue #4 fix: ref keeps finishExam current so the timer interval always
  // calls the latest instance, regardless of which deps triggered a re-create.
  ...
  // Issue #4 fix: keep finishExamRef always pointing to the latest finishExam.
  // This assignment is synchronous on every render, so the ref is always current
  // before any effect or interval fires.
  ...
  // Issue #4 fix: calls finishExamRef.current() so the interval always invokes
  // the latest finishExam regardless of when it was last re-created.
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          finishExamRef.current();   // Issue #4: call via ref, not stale closure
          return 0;
        }
        const next = prev - 1;
