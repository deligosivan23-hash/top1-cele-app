...
export default function ExamSessionScreen() {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          if (!doneRef.current) { doneRef.current = true; finishExam(); }
          return 0;
        }
        return prev - 1;
      });
  ...
  const finishExam = useCallback((forcedAnswers) => {
    if (doneRef.current && !forcedAnswers) return;
    doneRef.current = true;
    clearInterval(timerRef.current);

    const finalAnswers = forcedAnswers ?? answers;
    const qs           = examConfig.questions;
    const timeUsedMins = Math.max(1, Math.round((totalSecs - timeLeft) / 60));
    ...
    qs.forEach((q, i) => {
      ...
      // Log to topic stats
      logAttempt(tid, isCorrect, 90); // use 90s as representative exam time
    ...
    const result = {
      ...
      totalCorrect,
      weakTopics,
      questionResults,   // full per-question data for review
      byCat,
    };
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [answers, examConfig, timeLeft, totalSecs]);
