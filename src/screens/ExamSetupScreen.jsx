...
function generateExamQuestions(type, count) {
  ...
  if (type === 'Full') {
    ...
    // Bug #9 fix: recycle combined pool if any category was smaller than perCat
    let combined = shuffle([...mste, ...hge, ...sec]);
    while (combined.length < count) combined = [...combined, ...shuffle([...mste, ...hge, ...sec])];
